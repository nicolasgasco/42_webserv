...
int main(int argc, char **argv)
{
	...
	else
	{
		try
		{
			...
			for (std::vector<Server>::const_iterator it = webserver.get_server().begin(); it != webserver.get_server().end(); it++)
			{
				Server srv_data = *it;

				// TODO change this with real logic
				std::string port = "80";
				std::string host_name = srv_data.get_host();

				AddressInfo addr_info(port, host_name);
				srv_data.set_addr_info(addr_info);

				Socket socket(addr_info);
				srv_data.set_socket(socket.get_socket_id());

				SocketConnection sock_connection(srv_data.get_socket(), addr_info, BACKLOG_DEFAULT);
				std::cout << YELLOW << "Socket listening on port " << srv_data.get_socket() << "..." << NC << std::endl;

				// Set server_socket for reading
				FD_SET(srv_data.get_socket(), &read_fds_cpy);

				max_fd = srv_data.get_socket() > max_fd ? srv_data.get_socket() : max_fd;
			}
				...
				for (int i = MIN_FD; i <= max_fd; ++i)
				{
					...
					if (FD_ISSET(i, &read_fds))
					{
						for (std::vector<Server>::const_iterator it = webserver.get_server().begin(); it != webserver.get_server().end(); it++)
						{
							...
							if (i == srv_data.get_socket())
							{
								int client_fd = connections.at(i).accept_connection(i, srv_data.get_addr_info().get_serv_info());
